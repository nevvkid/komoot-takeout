...
def get_collections():
    """Get user's collections and the tours within them"""
    try:
        ...
        logger.info(f"Getting collections for user {email}")
        
        # Validate input
        if not email or not password:
            return jsonify({'error': 'Email and password are required'}), 400
            
        # Login and fetch collections
        try:
            logger.info("Logging in to Komoot...")
            komoot_adapter.login(email, password)
            logger.info(f"Logged in successfully as {komoot_adapter.get_display_name()}")
            
            collections = komoot_adapter.fetch_collections()
            logger.info(f"Found {len(collections)} collections")
            
            # Generate summary
            total_tours = sum(collection.get('tours_count', 0) for collection in collections)
            
            # Create a CSV export of collections
            csv_data = "Collection Name,Collection Type,Tour Count\n"
            for collection in collections:
                csv_data += f"\"{collection['name']}\",\"{collection['type']}\",{collection['tours_count']}\n"
            
            # Create a CSV export of tours in collections
            tours_csv_data = "Collection Name,Tour ID,Tour Name,Tour URL\n"
            for collection in collections:
                collection_name = collection['name']
                for tour in collection.get('tours', []):
                    tours_csv_data += f"\"{collection_name}\",{tour['id']},\"{tour['name']}\",{tour['url']}\n"
            ...
            response_data = {
                ...
                'csv': csv_data,
                'tours_csv': tours_csv_data,
                'success': True
            }
            
            # Save the collection data for later use
            app.last_collection_data = response_data
...
@app.route('/api/fetch-collection', methods=['POST'])
def fetch_collection():
    """Fetch a specific collection by its URL"""
    try:
        # Get parameters from request
        data = request.json
        if data is None:
        ...
        if not collection_url:
            return jsonify({'error': 'Collection URL is required'}), 400
            
        if not collection_url.startswith('https://www.komoot.com/collection/'):
            return jsonify({'error': 'Invalid collection URL. Must start with https://www.komoot.com/collection/'}), 400
            
        # Login and fetch collection
        try:
            logger.info("Logging in to Komoot...")
            komoot_adapter.login(email, password)
            logger.info(f"Logged in successfully as {komoot_adapter.get_display_name()}")
            
            collection = komoot_adapter.fetch_collection_by_url(collection_url)
            
            if not collection:
                return jsonify({'error': 'Failed to fetch collection'}), 500
                
            # Generate CSV for tours in this collection
            tours_csv = "Tour ID,Tour Name,Tour URL\n"
            for tour in collection.get('tours', []):
                tours_csv += f"{tour['id']},\"{tour['name']}\",{tour['url']}\n"
            ...
            logger.error(f"Error in Komoot operations: {str(e)}")
            return jsonify({'error': f'Error fetching collection: {str(e)}'}), 500
...
@app.route('/api/export/collections')
def export_collections_csv():
    """Export collections data as CSV files"""
    try:
        import zipfile
        
        # Check if we have collection data
        if not hasattr(app, 'last_collection_data') or not app.last_collection_data:
            return jsonify({'error': 'No collection data available. Please fetch collections first.'}), 400
        
        # Create a zip file in memory
        memory_file = io.BytesIO()
        with zipfile.ZipFile(memory_file, 'w') as zf:
            # Add collections summary CSV
            zf.writestr('collections_summary.csv', app.last_collection_data.get('csv', ''))
            
            # Add tours in collections CSV
            zf.writestr('tours_in_collections.csv', app.last_collection_data.get('tours_csv', ''))
        ...
        return send_file(
            memory_file,
            as_attachment=True,
            download_name=f'komoot_collections_{timestamp}.zip',
            mimetype='application/zip'
        )
    
    except Exception as e:
        logger.error(f"Error creating collections export: {e}")
        return jsonify({'error': f'Error creating collections export: {str(e)}'}), 500
